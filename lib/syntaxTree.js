...
function SyntaxTree(input, fields, exps) {
//CONFIG
var operands = ["then",  "plus", "equals", "or", "and", "was", "was not", "is", "is not", "is now", "has changed", "greater than", "less than"];
var to_delete = ["if"];
...
while (words.length) {
  ...
  while (len--) {
      ...
      //CHECK FOR MATCH
      var inProps = fields.hasOwnProperty(str);
      var inOperands = isIn(operands, str);
      var inExps =  exps.hasOwnProperty(str);
...
function PlusNode() {
  this.type = "plus";
  this.interpret = function(state) {
      this.left_child.interpret(state),
      this.right_child.interpret(state);
      return true;
  }
}
function HasChangedNode(){
  ...
  this.interpret = function(state){
     console.log("LEF TNODE", this.left_child.type)
     console.log("RIGHT TNODE", this.right_child.type)
      var then = this.left_child.interpret(state, true);
      var now = this.left_child.interpret(state);
      console.log("COMPARE", then, now)
      return now != then;
  }
}
function NoOpNode() {
