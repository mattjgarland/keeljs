...
function enforceTriplets(nodes) {
  ...
  for (var i = 0; i < nodes.length; i++) {
    let node = nodes[i]
    if (isIn(getSwapRight(), node.type)) {
  ...
  for (var i = 0; i < nodes.length; i++) {
    let node = nodes[i]
    if (isIn(getSwapRightAndAddRight(), node.type)) {
      let nextNode = nodes[i + 1]
      nodes[i + 1] = node
      nodes[i] = nextNode
...
function nodesToTree(nodes) {
  ...
  while (nodes.length > 1) {
    loop++
    if (loop > maxLoops){
      throw new Error("Sibling operand nodes (eg. 2 thens) cannot resolve to single tree.")
    }
...
function makeOperandNode(w, nodes) {
  ...
  if (w == "and") return new AndNode()//CompareNode(w, and)
